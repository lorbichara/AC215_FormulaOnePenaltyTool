...
machine_disk_size = 20  # Further reduced to fit within 250GB quota


def create_cluster(project, zone, network, subnet, app_name):  # Changed parameter name from region to zone
    # Create GKE cluster with private nodes, workload identity enabled, and no default node pool
    cluster = gcp.container.Cluster(
        f"{app_name}-cluster",
        name=f"{app_name}-cluster",
        description="GKE cluster",
        location=zone,  # Use zone for zonal cluster
    ...
    node_pool = gcp.container.NodePool(
        ...
        location=zone,  # Use zone instead of region
        initial_node_count=1,
        ...
        # Remove autoscaling entirely to reduce quota reservation
    ...
    connect_k8s_command = command.local.Command(
        ...
        create=Output.concat(
            ...
            cluster.name,
            f" --zone {zone} --project {project}"  # Use zone variable
        ),
    )
