...
def main():
    ...
    parser.add_argument(
        "-l", "--limit", type=int, help="Limit the number of documents to download."
    )
    parser.add_argument(
        "--steps",
        choices=["scrape", "convert", "all"],
        default="all",
        help="Choose which steps to run.",
    )
    parser.add_argument(
        "--upload_to_gcs",
        action="store_true",
        help="Upload files to GCS instead of downloading locally.",
    )
    parser.add_argument(
        "--bucket",
        type=str,
        default="f1penaltydocs",
        help="The GCS bucket name to upload files to.",
    )
